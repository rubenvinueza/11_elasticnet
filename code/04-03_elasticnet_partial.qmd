...
Ridge never brings any coefficient to cero. 
...
```
How does metrics on test compare to metrics on train? Why?
R square is higher on train than test set. You expect model to make better predictions on train set because it was trained for that. Metrics are expected to be better. 

Predicted vs. observed plot:  check this code because is incomplete
```{r}
final_fit %>%
  collect_predictions() %>%
